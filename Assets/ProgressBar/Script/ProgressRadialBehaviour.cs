...
namespace ProgressBar
{
    ...
    public class ProgressRadialBehaviour : MonoBehaviour, IIncrementable, IDecrementable
    {
        ...
        /// </summary>
		public static bool encryptionActive;
        public float Value
        {
            get
            {
		...
		public bool UpdateAttachedText;
        ...
        {
			if (AttachedText && UpdateAttachedText) {
				if (encryptionActive == true)
					AttachedText.text = Mathf.RoundToInt (fill * 100).ToString () + " percent Encrypted";
				else if (encryptionActive == false)
					AttachedText.text = Mathf.RoundToInt (fill * 100).ToString () + " percent Decrypted";
			}
